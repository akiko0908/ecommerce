    ...
    public class ManageProductController : Controller
    {
        ...
        public IActionResult Create(Product newProduct, IFormFile image)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var path = Path.Combine(Directory.GetCurrentDirectory(), "~/images/", image.FileName);
                    var stream = new FileStream(path, FileMode.Create);
                    image.CopyToAsync(stream);
                    newProduct.product_Image = image.FileName;
                }
        ...
        {
            var oldProduct = dbContext.Products.Include(p => p.Brand)
        ...
        public IActionResult ConfirmDelete(int? id)
        {
            if (id != null)
            {
                ...
                dbContext.Remove(chooseProduct);
                dbContext.SaveChanges();
                return RedirectToAction("Index");
            }
        ...
        public IActionResult Edit(int? id, Product newProduct)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                oldProduct.product_Image = newProduct.product_Image;
